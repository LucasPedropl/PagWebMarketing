...
export const PlansPage: React.FC<PlansPageProps> = ({ variant = 'desktop' }) => {
  ...
  return (
    <div className={`flex flex-col h-full bg-white ${isMobile ? 'p-4 pb-24' : 'p-8'}`}>
        ...
        {/* Grid Section - Adjusted for Mobile Variant */}
        {/* We conditionally apply grid only if NOT mobile to prevent the high-res scaling in Hero from triggering md:grid */}
        <div className={`
            flex flex-col gap-6 pb-4 animate-in fade-in slide-in-from-bottom-4 duration-500
            ${isMobile ? '' : 'md:grid md:grid-cols-2 lg:grid-cols-3'}
        `}>
            
            {/* Card 1: Basic */}
            <div className="bg-white border border-gray-200 rounded-xl p-6 hover:shadow-md transition-all duration-200 flex flex-col w-full group relative shrink-0">
            ...
            <div className="bg-white border-2 border-slate-900 rounded-xl p-6 shadow-xl flex flex-col w-full relative shrink-0">
            ...
            {/* Card 3: Add New */}
            <div className="border-2 border-dashed border-gray-200 rounded-xl p-6 flex flex-col items-center justify-center text-gray-400 hover:border-brand-300 hover:text-brand-600 hover:bg-brand-50 transition-all cursor-pointer w-full min-h-[160px] shrink-0">
